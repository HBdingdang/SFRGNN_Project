...
class PyramidSelfAttention(nn.Module):
    def __init__(self, in_channels, reduction=16):
        super(PyramidSelfAttention, self).__init__()
...
class EPSANetBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        super(EPSANetBlock, self).__init__()
        self.conv1 = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=1)
        self.bn1 = nn.BatchNorm2d(out_channels)
        self.relu = nn.ReLU(inplace=True)
        self.psa = PyramidSelfAttention(out_channels)
    ...
    def forward(self, x):
        ...
        if residual.shape != out.shape:
            ...
            residual = nn.Conv2d(residual.shape[1], out.shape[1], kernel_size=1)(residual)
...
class SFRGraphEncoder(nn.Module):
    def __init__(self, node_dim, edge_dim, num_layers, delta, mlp_ratio=4, drop=0., drop_path=0., conv_on_edge=True):
        super(SFRGraphEncoder, self).__init__()
        self.num_layers = num_layers
        self.conv_on_edge = conv_on_edge
        self.node_convs = nn.ModuleList()
        self.edge_convs = nn.ModuleList()

        # Add EPSANet Block for node feature preprocessing
        self.epsa_block = EPSANetBlock(node_dim, node_dim)

        # since 2nd layer, the subsequent layers are share-weight
        for _ in range(2):
            if self.conv_on_edge:
                self.edge_convs.append(EdgeMPNN(node_dim, edge_dim, mlp_ratio, drop, drop_path))
            self.node_convs.append(NodeMPNN(node_dim, edge_dim, delta, mlp_ratio, drop, drop_path))

        self.post_norm = nn.LayerNorm(node_dim)
        self.pool_avg = dgl.nn.AvgPooling()
    ...
    def forward(self, g, h, he):
        ...
        h = h.permute(0, 3, 1, 2)
        h = self.epsa_block(h)
        h = h.permute(0, 2, 3, 1).squeeze(2).squeeze(1)

        # print("h shape after EPSANetBlock:", h.shape)

        if h.size(-1) != self.node_convs[0].conv.in_size:
            h = torch.cat([h, torch.zeros(h.size(0), self.node_convs[0].conv.in_size - h.size(-1)).to(h.device)], dim=-1)

        if self.conv_on_edge:
            he = self.edge_convs[0](g, h, he)
            if he.size(-1) != self.edge_convs[0].proj[1].in_features:
                he = torch.cat([he, torch.zeros(he.size(0), self.edge_convs[0].proj[1].in_features - he.size(-1)).to(he.device)], dim=-1)
        h = self.node_convs[0](g, h, he)

        for i in range(self.num_layers - 1):
            if self.conv_on_edge:
                he = self.edge_convs[1](g, h, he)
                if he.size(-1) != self.edge_convs[1].proj[1].in_features:
                    he = torch.cat([he, torch.zeros(he.size(0), self.edge_convs[1].proj[1].in_features - he.size(-1)).to(he.device)], dim=-1)
            h = self.node_convs[1](g, h, he)

        local_feat = self.post_norm(h)
        # print("local_feat shape:", local_feat.shape)

        global_feat_avg = self.pool_avg(g, local_feat)
        global_feat_max = self.pool_max(g, local_feat)
        global_feat = torch.cat([global_feat_avg, global_feat_max], dim=1)
    ...
    def __init__(self,
        ...
        for layer in range(num_layers - 1):  # excluding the input layer
            ...
            self.ginlayers.append(
                dgl.nn.GINConv(mlp, learn_eps=False, aggregator_type='max'))  # set to True if learning epsilon
