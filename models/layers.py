...
#             g.edata["h"] = edge_feats
#
#             # Ensure dimensions match before applying edges
#             assert node_feats.shape[-1] == edge_feats.shape[-1], "Node and edge features must have the same dimension"
#
#             g.apply_edges(fn.u_add_e("h", "h", "m"))
#
#             if self.aggr == "softmax":
...
class MappingAndAffineModule(nn.Module):
    def __init__(self, in_features, style_features, out_features=10):  # 假设期望输出维度为10
        super().__init__()
        self.mapping = nn.Sequential(
            nn.Linear(in_features, style_features),
            nn.ReLU(inplace=True),
            nn.Linear(style_features, style_features),
            nn.ReLU(inplace=True)
        )
        self.affine = nn.Linear(style_features, out_features)

    def forward(self, x):
        style = self.mapping(x)
        return self.affine(style)
