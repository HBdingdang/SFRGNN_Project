...
class DomainAdversarialLoss(nn.Module):
    ...
    def forward(self, f_s: torch.Tensor, f_t: torch.Tensor,
        ...
        # Ensure the shapes of d_s and d_t match the shapes of d_label_s and d_label_t
        d_s, d_t = d[:f_s.size(0)], d[f_s.size(0):]
        d_label_s = torch.ones((f_s.size(0), 1)).to(f_s.device)
        d_label_t = torch.zeros((f_t.size(0), 1)).to(f_t.device)

        # # 打印调试信息
        # print(f"d_s shape: {d_s.shape}, d_label_s shape: {d_label_s.shape}")
        # print(f"d_t shape: {d_t.shape}, d_label_t shape: {d_label_t.shape}")
